...
IR Slave Receiver V0.3 (23.10.15)
...
#define DELETELAST     500  //µS Delete last remembered ID

#define MINSPACE      5000  //ms to register a new completet lap
...
{ 
  ...
  if(dataReady0 && micros() >= (dataReadTime0 + ANALYSEDELAY))  //New message is ready
  {
    ...
    {      
      ...
      if(checksum0 == message0[2 + DATABITS])  //is caculated checksum the same as read checksum?
      {
        ...
        if(data0 == lastPilot)   //ID has to be read 2 times to avoid single mistaken readings
        {
  ...
  if(millis() >= (lastValue + DELETELAST))
    lastPilot = 255;
}

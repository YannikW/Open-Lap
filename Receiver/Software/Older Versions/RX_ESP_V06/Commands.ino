...
void initiateCommands()
{
  ...
  TCPCmd.addDefaultHandler(unrecognized);         //Unknown Command input
}
...
void fpvRaceTrackerTCP()
{
  mode = 4; //select FPV Race Tracker output mode
  uint32_t startTime = millis();
  for(uint8_t a = 0; a < 64; a++) //save start time for every pilot
  {
    pass[a] = startTime;
...
void help()
{
  ...
  Serial.println("\"NEW_OPEN_LAP\" : \t Start new OpenLap Flying-Start-Race");
...
void unrecognized()
{
  ...
  client.println("Typ in \"HELP\" to show commands");
  SCmd.clearBuffer();
  TCPCmd.clearBuffer();
}

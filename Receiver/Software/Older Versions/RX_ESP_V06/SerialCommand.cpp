/*
 * This version is not original.
 * Its edited for OpenLap to work with a wifi connection.
 */
...
SerialCommand::SerialCommand(WiFiClient &_wifi)
{
...
void SerialCommand::readSerial() 
{
	...
	#else
	while ((usingSoftwareSerial==0 && Serial.available() > 0) || (usingSoftwareSerial==1 && wifiClient->available() > 0) )
	#endif
	{
		int i; 
		boolean matched; 
		if (usingSoftwareSerial==0) {
			...
			#ifndef SERIALCOMMAND_HARDWAREONLY
			// SoftwareSerial port
			inChar = wifiClient->read();   // Read single available character, there may be more waiting
			#endif
		}
		...
		if (isprint(inChar) || inChar == 1)   // Only printable characters into the buffer
		{

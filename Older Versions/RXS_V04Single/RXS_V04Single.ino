...
IR Slave Receiver V0.4 (24.10.15) Single Sensor Version
...
#define VERSION      "0.4"  //Version number

#define ID              10  //I2C adress. 10 ist first adress. For more slaves increment adress and configure master
...
uint8_t lastPilot[NUMSENSORS][2]; //Save last two values for colision avoidance
    ...
    if(dataReady[sensor] && micros() >= (dataReadyTime[sensor] + ANALYSEDELAY))  //New message is ready
    {
      ...
      {      
        ...
        if(checksum[sensor] == message[sensor][2 + DATABITS])  //is caculated checksum the same as read checksum?
        {
          ...
          if(twoOfThree(sensor, data[sensor]))   //ID has to be read 2 times to avoid single mistaken readings
          {
...
boolean twoOfThree(uint8_t sensor, uint8_t id)
{
  if(id == lastPilot[sensor][0] || id == lastPilot[sensor][1])
    return true;
  else
    return false;
}
